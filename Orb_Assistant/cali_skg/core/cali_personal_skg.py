...
def _default_base_path() -> str:
    substrate_root = os.getenv("CALI_CRM_SUBSTRATE_ROOT", "").strip()
    if substrate_root:
        return substrate_root
    configured = os.getenv("CALI_SKG_BASE_PATH", "").strip()
    if configured:
        return configured
    return str(Path(__file__).resolve().parents[2] / "CALI_System")
...
class CaliPersonalSKG:
    ...
    def _init_database(self) -> None:
        with self._connect() as conn:
            ...
            cur.execute("PRAGMA table_info(contacts)")
            existing = {str(row[1]) for row in cur.fetchall()}
            for column, column_type in additional_contact_columns.items():
                if column not in existing:
                    cur.execute(f"ALTER TABLE contacts ADD COLUMN {column} {column_type}")
            ...
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS crm_dataset_imports (
                    id TEXT PRIMARY KEY,
                    source_app TEXT NOT NULL,
    ...
    def _orb_cali_system_root(self) -> Path:
        ...
        if self.base_path.name == "CALI_System":
    ...
    def _find_contact_by_external_id(self, source_app: str, external_id: str) -> Optional[Dict[str, Any]]:
        source_app = str(source_app or "").strip()
        external_id = str(external_id or "").strip()
        if not source_app or not external_id:
            return None
        with self._connect() as conn:
            cur = conn.cursor()
            cur.execute(
                ...
                SELECT c.*
                FROM contacts c
                JOIN contact_external_links l ON l.contact_id = c.id
                WHERE l.source_app = ? AND l.external_id = ?
                LIMIT 1
                """,
    ...
    def _upsert_external_link(
        ...
        now = datetime.utcnow().isoformat()
        link_id = self._generate_hash(
            {"contact_id": contact_id, "source_app": source_app, "external_id": external_id}
        )
        with self._connect() as conn:
            cur = conn.cursor()
            cur.execute(
                ...
                INSERT INTO contact_external_links (
                    id, contact_id, source_app, source_entity_type, external_id,
    ...
    def import_external_contact_record(
        self,
        source_app: str,
        external_id: str,
        name: str,
        email: Optional[str] = None,
        phone: Optional[str] = None,
        source_entity_type: str = "customer",
        source_url: Optional[str] = None,
        metadata: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        metadata = metadata or {}
        ...
        source_app = str(source_app or "unknown").strip()
        external_id = str(external_id or "").strip() or self._generate_hash(
            {"source_app": source_app, "name": name, "email": normalized_email}
        )
        display_name = str(name or "").strip() or f"{source_app} contact"

        existing = self._find_contact_by_external_id(source_app, external_id)
        if not existing and normalized_email:
            existing = self._find_contact_by_email(normalized_email)

        contact_type = "business" if source_entity_type in {"customer", "orb_weaver_customer", "account"} else "marketing"
        lead_source = str(metadata.get("lead_source") or source_app)
        owner = str(metadata.get("owner") or "spruked.com")
        notes = str(metadata.get("notes") or f"Imported from {source_app}.").strip()

        if existing:
            contact_id = str(existing["id"])
            merged_notes = self._append_notes(existing.get("notes"), notes)
            with self._connect() as conn:
                cur = conn.cursor()
                cur.execute(
                    """
            ...
            created_result = self.add_contact(
                name=display_name,
                contact_type=contact_type,
                phone=phone,
                email=normalized_email or None,
                notes=notes,
                priority=int(metadata.get("priority") or 2),
                crm_stage=str(metadata.get("crm_stage") or "prospect"),
            ...
            with self._connect() as conn:
                cur = conn.cursor()
                cur.execute(
                    "UPDATE contacts SET metadata_payload = ?, updated_at = ? WHERE id = ?",
                    (json.dumps(metadata), now, contact_id),
                )
                conn.commit()
            created = True
        ...
        self.log_crm_activity(
            ...
            f"Imported/updated {source_entity_type} record from {source_app}.",
            metadata={"source_app": source_app, "external_id": external_id, **metadata},
        )
        return {"success": True, "created": created, "contact_id": contact_id, "external_id": external_id}
    ...
    def import_external_dataset(
        ...
        for record in records:
            record_meta = {**metadata, **(record.get("metadata") if isinstance(record.get("metadata"), dict) else {})}
            result = self.import_external_contact_record(
                source_app=source_app,
                external_id=str(record.get("external_id") or record.get("id") or ""),
                name=str(record.get("name") or record.get("full_name") or record.get("email") or ""),
                email=record.get("email"),
                phone=record.get("phone"),
                source_entity_type=str(record.get("source_entity_type") or record.get("entity_type") or "customer"),
                source_url=record.get("source_url"),
                metadata=record_meta,
            )
            imported.append(result)

        import_id = self._next_id(
            "dataset_import",
            {"source_app": source_app, "dataset_name": dataset_name, "count": len(records)},
        )
        now = datetime.utcnow().isoformat()
        with self._connect() as conn:
            cur = conn.cursor()
            cur.execute(
                """
                INSERT INTO crm_dataset_imports (
                    id, source_app, dataset_name, external_id, import_status,
                    record_count, result_contact_id, metadata, created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    ...
                    "imported",
                    len(records),
                    str(imported[0].get("contact_id") if imported else ""),
                    json.dumps({"metadata": metadata, "results": imported}),
                    now,
                ),
            )
            conn.commit()
        ...
        self._log_memory(
            "external_dataset_import",
            {"source_app": source_app, "dataset_name": dataset_name, "count": len(records), "results": imported},
            confidence=0.95,
            source=source_app,
        )
        return {"success": True, "import_id": import_id, "imported": imported, "count": len(imported)}
