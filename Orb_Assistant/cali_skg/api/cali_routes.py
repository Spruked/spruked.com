...
import httpx
import cali_skg as cali_skg_package
...
from cali_skg.core.orb_cognition_skg import CALISKG
from cali_skg.core.cali_state_v4 import StateBundle, StateLayer
...
_CALI_V4: Any | None = None
_CALI_V4_LOCK = threading.Lock()
_ORB_ASSISTANT_ROOT = Path(__file__).resolve().parents[2]
_CALI_V4_STATE = StateBundle(
    persist_path=Path(os.getenv("CALI_V4_STATE_PATH", str(_ORB_ASSISTANT_ROOT / "CALI_System/state/cali_state_v4.json")))
)
_CALI_V4_STATE.load()
_ORB_VAULTS = CoreVaults()


def _get_cali_v4() -> Any:
    """Return the process-wide CALI v4 reasoning engine used by the website ORB."""
    global _CALI_V4
    if _CALI_V4 is None:
        with _CALI_V4_LOCK:
            if _CALI_V4 is None:
                # CALISKG appends "CALI_System" internally, so its owner root is
                # Orb_Assistant. This connects the existing authoritative vaults
                # and patterns DB instead of creating a parallel memory tree.
                system_path = Path(os.getenv("CALI_V4_SYSTEM_PATH", str(_ORB_ASSISTANT_ROOT)))
                _CALI_V4 = CALISKG(system_path=system_path)
                _CALI_V4.state_bundle = _CALI_V4_STATE
                _CALI_V4.orb_state = _CALI_V4_STATE.recompute_all_effective()
    return _CALI_V4


def _reason_with_cali_v4(prompt: str, context: Dict[str, Any]) -> Dict[str, Any]:
    _CALI_V4_STATE.set("listener_active", True, layer=StateLayer.ACTIVITY, source="website_orb")
    _CALI_V4_STATE.set("current_path", str(context.get("current_path") or "/"), layer=StateLayer.RUNTIME, source="website_orb")
    try:
        result = _get_cali_v4().reason(prompt, context=context)
        _CALI_V4_STATE.set("last_reasoning_timestamp", result.get("timestamp"), layer=StateLayer.RUNTIME, source="cali")
        _CALI_V4_STATE.persist()
        return result
    finally:
        _CALI_V4_STATE.set("listener_active", False, layer=StateLayer.ACTIVITY, source="website_orb")
...
def _ollama_model_name() -> str:
    return str(os.getenv("CALI_OLLAMA_MODEL_NAME", "qwen2.5:3b")).strip()


def _llm_base_url(base_url: Optional[str] = None) -> str:
    return str(
        base_url
        or os.getenv("CALI_LLM_BASE_URL")
        or os.getenv("LLAMA_SERVER_BASE_URL")
...
def _is_llama_cpp_base(base_url: str) -> bool:
    lowered = base_url.lower()
    return (
        "8012" in lowered
        or "llama" in lowered
        or str(os.getenv("CALI_LLM_PROVIDER", "")).strip().lower() in {"llama", "llamacpp", "llama.cpp"}
    )
...
def _fast_path_response(prompt: str) -> str:
    ...
    test_remainder = re.sub(r"[\s,?.!;:\-]*(testing|test|or)[\s,?.!;:\-]*", "", prompt_lower)
    if re.search(r"\b(test|testing)\b", prompt_lower) and not test_remainder:
        return "I hear you clearly. The voice test is working."
    if re.search(r"\b(what(?:'s| is)? your name|who are you)\b", prompt_lower):
        return "I'm Cali. I'm here with you."
    if re.search(r"\b(primary function|primary role|your role|your purpose|what do you do)\b", prompt_lower):
        return (
...
def _generate_llm_response(prompt: str, context: Dict[str, Any], emotion: str, llm_base_url: Optional[str] = None) -> str:
    model = _ollama_model_name()
    base_url = _llm_base_url(llm_base_url)
    system_prompt = (
        "You are Cali, a female executive assistant for Bryan on spruked.com. "
        "Spruked's current product surfaces include TrueMark Mint for verified object records and provenance, "
        "Alpha CertSig, Prompt Like a Pro, and Orb Skin Studio. "
        "Follow CALI SKG governance style: concise, calm, practical, and safe. "
        "Answer the user's actual message directly in natural language. "
        "The four philosophical minds and three system logics are private advisory machinery; "
        "never narrate their scores, call a system logic a mind, or say that a lens offers the strongest frame. "
        "Never output diagnostic fields like MIND or CONF."
    )
    context_hint = ""
    advisory = context.get("cali_v4_advisory") if context else None
    if isinstance(advisory, dict):
        context_hint = "\nPrivate governance: the Core Four authorized direct articulation. Do not expose internal reasoning."
    full_prompt = f"Emotion: {emotion}\nUser: {prompt}{context_hint}"

    try:
        if _is_llama_cpp_base(base_url):
            response = httpx.post(
        ...
        if _is_llama_cpp_base(base_url):
            ...
            response_text = message.get("content") or first.get("text") or ""
        else:
            response_text = result.get("response") or result.get("thinking", "")
        return str(response_text or "").strip()
    except Exception as exc:
        raise RuntimeError(f"LLM API call failed at {base_url}: {exc}") from exc
...
class ExternalDatasetImport(BaseModel):
    source_app: str
    dataset_name: str
...
class OrbWeaverCustomerSignup(BaseModel):
    customer_id: str
    email: str
    name: Optional[str] = None
    domain: str = "spruked.com"
...
def _with_orb_memory(
    ...
    try:
        ...
        posteriori = _ORB_VAULTS.append_posteriori(content, confidence=confidence, source="website_orb")
        daily_file = _ORB_VAULTS.memory_root / f"orb_memory_{datetime.utcnow().strftime('%Y-%m-%d')}.jsonl"
        record = {
            "timestamp": datetime.utcnow().isoformat(),
            "category": "orb_exchange",
            "content": content,
            "hash_id": hashlib.sha256(json.dumps(content, sort_keys=True).encode("utf-8")).hexdigest()[:16],
        ...
        result["memory"] = {
            ...
            "runtime": "orb_cognition_singleton",
        }
    except Exception as exc:
...
@router.post("/imports/datasets")
def import_external_dataset(payload: ExternalDatasetImport, _: str = Depends(verify_admin)) -> Dict[str, Any]:
    cali = get_cali_skg()
    return cali.import_external_dataset(
...
@router.post("/imports/orb-weaver/customer-signup")
def import_orb_weaver_customer_signup(
    payload: OrbWeaverCustomerSignup,
    ...
    metadata = {
        ...
        "lead_source": "orb_weaver_signup",
        "owner": "spruked.com",
        **(payload.metadata or {}),
    }
    return cali.import_external_dataset(
        source_app="orb_weaver",
        dataset_name="customer_signup",
        records=[
            {
                "external_id": payload.customer_id,
                "name": payload.name or payload.email,
                "email": payload.email,
                "source_entity_type": "orb_weaver_customer",
                "source_url": payload.source_url,
                "metadata": metadata,
            }
        ],
        metadata={"external_id": payload.customer_id, "domain": payload.domain},
    )
...
@router.get("/runtime/proof")
def runtime_proof(_: str = Depends(verify_admin)) -> Dict[str, Any]:
    engine = _get_cali_v4()
    vault_module = sys.modules.get(_ORB_VAULTS.__class__.__module__)
    core_reasoner_module = sys.modules.get("cali_skg.core.core_mind_reasoner")
    if core_reasoner_module is None:
        __import__("cali_skg.core.core_mind_reasoner")
        core_reasoner_module = sys.modules.get("cali_skg.core.core_mind_reasoner")
    return {
        "cali_skg_file": str(Path(cali_skg_package.__file__).resolve()),
        "engine_class": f"{engine.__class__.__module__}.{engine.__class__.__name__}",
        "engine_module_file": str(Path(sys.modules[engine.__class__.__module__].__file__).resolve()),
        "state_bundle_module": _CALI_V4_STATE.__class__.__module__,
        "state_bundle_id": id(_CALI_V4_STATE),
        "engine_state_bundle_id": id(engine.state_bundle),
        "single_state_bundle": engine.state_bundle is _CALI_V4_STATE,
        "core_four_reasoner_file": str(Path(core_reasoner_module.__file__).resolve()),
        "vault_module_file": str(Path(vault_module.__file__).resolve()),
        "a_priori_vault": str(_ORB_VAULTS.a_priori_file.resolve()),
        "a_posteriori_vault": str(_ORB_VAULTS.a_posteriori_file.resolve()),
        "sovereign_posteriori": str(_ORB_VAULTS.posteriori_object_root.resolve()),
        "patterns_database": str((engine.cali_root / "memory/patterns.db").resolve()),
        "personal_database": str((_ORB_VAULTS.memory_root / "cali_personal.db").resolve()),
        "authority": {
            "decision_maker": "core_four",
            "system_logics": ["inductive", "deductive", "intuitive"],
...
async def cali_orb_respond(payload: OrbRespondRequest) -> Dict[str, Any]:
    ...
    skg_result = await run_in_threadpool(_reason_with_cali_v4, prompt, skg_context)
    v4_reasoning = list(skg_result.get("philosophical_reasoning") or [])
    v4_advisory = dict(skg_result.get("advisory_verdict") or {})
    core_four_decision = dict(skg_result.get("core_four_decision") or {})
    leading_mind = str(core_four_decision.get("leading_mind") or "Immanuel Kant")
    core_reasoning = {
        "leading_mind": leading_mind,
        "philosophical_reasoning": v4_reasoning,
        "advisory_verdict": v4_advisory,
        "core_four_decision": core_four_decision,
        "reasoning_mode": str(skg_result.get("reasoning_mode") or ""),
        "engine": "Orb_Assistant.CALI_System.cali_skg.core.orb_cognition_skg",
    }
    if _is_substrate_query(prompt):
        ...
        return _with_orb_memory({
            ...
            "metadata": {
                "provider": "cali_skg",
                "cognition": "orb-canonical-cognition-state-v4",
                "llm_core": "substrate-redis-brief",
                "leading_mind": leading_mind,
                "confidence": 0.9,
                "truth_likelihood": 0.9,
                "core_reasoner": "Orb_Assistant.CALI_System.cali_skg.core.orb_cognition_skg",
                "state_engine": "cali_skg.core.cali_state_v4",
            },
        }, prompt, context)
    ...
    intent_type = "cali_v4_reasoning"
    llm_core = f"ollama:{_ollama_model_name()} + orb-core-four-advisory"
    response_text = ""
    ...
    if _use_local_llm_for_unknown():
        try:
            articulation_context = {
                **context,
                "cali_v4_advisory": {
                    "leading_lens": leading_mind,
                    "confidence": v4_advisory.get("confidence"),
                },
            }
            response_text = _generate_llm_response(
                prompt,
                context=articulation_context,
                emotion=str(payload.emotion or "thoughtful_warm"),
                llm_base_url=payload.llm_base_url or payload.ollama_base_url,
            )
        except Exception as exc:
            if _strict_mode():
                raise HTTPException(status_code=503, detail=f"Hybrid cognition unavailable: {exc}") from exc
            llm_core = "cali-v4-deterministic-fallback"

    if not response_text:
        response_text = str(skg_result.get("recommended_response") or "").strip()
